// Application-owned adaptation of Dear ImGui 1.91.9b DX12 backend (MIT).
// No ImGui context is accessed on the rendering thread.
...
static thread_local ImGuiIO renderIO;
static thread_local ImGuiPlatformIO renderPlatformIO;
void ConfigureRenderUI(ImFontAtlas* fonts) { renderIO.Fonts = fonts; }
...
{
    return (ImGui_ImplDX12_Data*)renderIO.BackendRendererUserData;
}
...
// Functions
static void ImGui_ImplDX12_SetupRenderState(UiDrawData* draw_data, ID3D12GraphicsCommandList* command_list, ImGui_ImplDX12_RenderBuffers* fr)
{
...
// Render function
void ImGui_ImplDX12_RenderDrawData(UiDrawData* draw_data, ID3D12GraphicsCommandList* command_list)
{
    ...
    {
        const UiDrawList* draw_list = &draw_data->CmdLists[n];
    ...
    ImGuiPlatformIO& platform_io = renderPlatformIO;
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.size(); cmd_i++)
        {
            ...
            if (pcmd->UserCallback != nullptr)
            {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
                    ...
                    IM_ASSERT(false && "UI callbacks must be converted to copied render commands");
            }
            else
            {
...
bool ImGui_ImplDX12_Init(ImGui_ImplDX12_InitInfo* init_info)
{
    ...
    ImGui_ImplDX12_Data* bd = new ImGui_ImplDX12_Data();
