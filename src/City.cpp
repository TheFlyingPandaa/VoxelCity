...
bool zone(BuildingKind k){return k>=BuildingKind::Residential&&k<=BuildingKind::Industrial;}
...
CitySimulation::CitySimulation(uint32_t seed):random_(seed?seed:42) {
    definitions_={BuildingDefinition{"None"},
        {"Residential zone",0,0,8,0,0},{"Commercial zone",0,0,6,0,0},{"Industrial zone",0,0,10,0,0},
        ...
        {"Police station",1800,15,2000,2000,64},{"School",2200,20,2000,2000,64},{"Park",500,5,2000,500,40}};
}
...
bool CitySimulation::buildRoad(World& w,Cell a,Cell b,bool erase,std::string& message) {
    if(!World::valid(a.x,a.z)||!World::valid(b.x,b.z))return false;
    std::vector<Cell> cells{a};int dx=std::abs(b.x-a.x),dz=std::abs(b.z-a.z),sx=b.x>a.x?1:-1,sz=b.z>a.z?1:-1,ix=0,iz=0;
    while(ix<dx||iz<dz){if(ix<dx&&(iz==dz||(1+2*ix)*dz<=(1+2*iz)*dx)){a.x+=sx;++ix;}else{a.z+=sz;++iz;}cells.push_back(a);}
    int changes=0;for(auto c:cells){if(erase&&w.highway(c)){message="The regional highway is maintained by neighboring cities. Remove local roads instead.";return false;}if(!erase&&at(c)){message="Occupied parcel: bulldoze it first.";return false;}if(w.road(c.x,c.z)!=!erase)++changes;}
    ...
    double cost=changes*(erase?5:20);if(!sandbox&&treasury<cost){message="Insufficient funds for this road stroke.";return false;}
    if(!changes)return false;for(auto c:cells)w.setRoad(c.x,c.z,!erase);if(!sandbox)treasury-=cost;++revision;accessDirty_=true;++accessRevision_;message=erase?"Road removed.":"Road built ($20 per tile).";return true;
}
bool CitySimulation::buildDiagonalRoad(World& w,Cell a,Cell b,std::string& message){
    ...
    auto clear=[&](Cell c){return World::valid(c.x,c.z)&&!at(c)&&!w.highway(c)&&w.roundaboutOrigin(c).x<0;};
...
bool CitySimulation::buildRoundabout(World& w,Cell o,std::string& message) {
    ...
    for(int z=0;z<3;++z)for(int x=0;x<3;++x){Cell c{o.x+x,o.z+z};if(w.roadOccupies(c)||at(c)||w.roundaboutOrigin(c).x>=0){message="Roundabout requires an empty 6 x 6 footprint.";return false;}}
...
bool CitySimulation::place(World& w,Cell c,BuildingKind kind,std::string& message) {
    ...
    if(w.roadOccupies(c)||at(c)){message="Parcel occupied.";return false;}
...
bool CitySimulation::bulldoze(World& w,Cell c,std::string& message) {
    ...
    auto b=at(c);if(!b)return buildRoad(w,c,c,true,message);
    ...
    if(i+1<buildings_.size()){buildings_[i]=buildings_.back();ids_[buildings_[i].id]=i;parcel_[tile(buildings_[i].cell)]=int(i);}buildings_.pop_back();
...
bool CitySimulation::roadRule(World& w,Cell c,uint8_t rule,std::string& message) {
    ...
    if(w.highway(c)){message="Regional highway directions are fixed. Build from its access junction.";return false;}
...
bool CitySimulation::computeAccess(World& w,size_t& cursor) {
    ...
    for(auto& b:buildings_){b.access={};b.component=-1;b.external=false;b.services.fill(0);
        for(int d=0;d<4;++d){Cell n{b.cell.x+DX[d],b.cell.z+DZ[d]};if(w.road(n.x,n.z)&&!w.highway(n)){int id=components_[tile(n)];if(b.component<0||exits_[id].x>=0){b.access=n;b.component=id;b.external=exits_[id].x>=0;if(b.external)break;}}}}
    ...
    for(size_t facilityIndex=cursor-1;facilityIndex<buildings_.size();++facilityIndex){const auto& facility=buildings_[facilityIndex];cursor=facilityIndex+2;if(facility.kind<BuildingKind::Garbage||facility.component<0)continue;
...
void CitySimulation::events(TrafficSimulation& traffic) {
    for(const auto& e:traffic.takeEvents()){
        ...
        if(e.completed){++stats_.completedTrips;tripSeconds_+=double(e.ticks)/30;
            if(t.kind==1){if(b){b->inventory+=t.cargo;if(!t.source&&!sandbox)treasury-=t.cargo*0.5;++stats_.deliveries;}else if(a){if(!sandbox)treasury+=t.cargo*0.5;++stats_.deliveries;}}
        ...
        }else {++stats_.failedTrips;if(t.kind==1&&a)a->inventory+=t.cargo;if(t.household)for(auto& h:households_)if(h.id==t.household)h.commuteQuality=0.25f;}
...
void CitySimulation::cityStep(World& w,TrafficSimulation& traffic) {
    ...
    for(const auto& b:buildings_){pop+=b.residents;workforce+=b.residents/2;if(b.level&&zone(b.kind)&&b.kind!=BuildingKind::Residential)jobs+=definition(b.kind).capacity*b.level;
        ...
        if(b.kind>=BuildingKind::Garbage)n.serviceCapacity[int(b.kind)-int(BuildingKind::Garbage)]+=definition(b.kind).capacity;
        if(zone(b.kind))n.demand+=std::max(4,b.kind==BuildingKind::Residential?b.residents:definition(b.kind).capacity*std::max(1,int(b.level)));
    }
    ...
    for(auto& b:buildings_){b.workers=0;b.productivity=0;if(b.level&&(b.kind==BuildingKind::Commercial||b.kind==BuildingKind::Industrial)&&b.component>=0)employers[b.component].push_back(&b);}
    ...
    for(auto& b:buildings_){++b.age;b.utilities.fill(0);
        ...
        if(!b.level&&b.component>=0&&b.external&&supply>=0.95f&&stats_.demand[int(b.kind)-1]>0.1f&&b.age>=5){b.level=1;b.distress=0;accessDirty_=true;++accessRevision_;if(b.kind==BuildingKind::Residential)households_.push_back({nextId_++,b.id,0,0});}
        if(!b.level){if(b.problem.empty())b.problem=b.age<5?"Preparing development":stats_.demand[int(b.kind)-1]<=0.1f?"Low demand for this zone":"Waiting for construction";continue;}
        ...
        float target=75*supply+service(5)*10-b.pollution*0.15f-b.waste*0.12f-b.crime*0.15f-b.fire*0.5f-std::max(0,taxes[int(b.kind)-1]-9)*3;
        if(!b.external)target-=25;if(b.kind==BuildingKind::Residential)target-=stats_.unemployment*25;
        ...
        if(b.kind==BuildingKind::Residential){
            ...
            if(second%3==b.id%3&&b.residents<cap&&b.external&&supply>0.95f&&b.happiness>45&&stats_.demand[0]>0.1f&&migrations<32){++b.residents;++migrations;}
            ...
            if(b.kind==BuildingKind::Commercial){if(b.inventory>0&&b.workers>0&&second%5==0)--b.inventory;else if(b.inventory==0){b.problem="Goods have not arrived";b.happiness=std::max(0.f,b.happiness-0.2f);}}
        }
    ...
    for(auto& b:buildings_)if(b.level&&b.component>=0){
        if(b.kind==BuildingKind::Commercial&&b.inventory<20&&second%15==b.id%15){Building* source=nullptr;
        ...
        if(b.kind==BuildingKind::Industrial&&b.inventory>200&&second%30==b.id%30)if(sendTrip(traffic,b.id,0,1,20))b.inventory-=20;
    ...
    double income=0,expenses=(w.roadCount()-w.highwayCount())*0.01+(loanTaken?250:0);
    for(const auto& b:buildings_){stats_.population+=b.residents;stats_.happiness+=b.happiness*b.residents;stats_.employed+=b.workers;
        if(zone(b.kind)){if(b.kind==BuildingKind::Residential)income+=b.residents*taxes[0]*0.12;else if(b.level){stats_.jobs+=definition(b.kind).capacity*b.level;income+=b.workers*b.productivity*taxes[int(b.kind)-1]*0.4*(b.kind==BuildingKind::Commercial&&b.inventory==0?0.2:1);}}
...
void CitySimulation::tick(World& w,TrafficSimulation& traffic){if(accessDirty_||topology_!=w.topologyRevision())rebuild(w);++ticks_;traffic.tick(w);events(traffic);if(ticks_%30==0)cityStep(w,traffic);}
...
void CitySimulation::publish(World& w){
    ...
    w.setTileTint(b.cell,tint);w.setParcelVisual(b.cell,{uint8_t(b.kind),b.level,b.variant,0});}}
...
void CitySimulation::save(World& w,TrafficSimulation& traffic,const std::filesystem::path& path) {
    ...
    traffic.saveState(out);std::string data=out.str();auto temporary=path;temporary+=L".tmp";
    try {std::ofstream f(temporary,std::ios::binary|std::ios::trunc);
        for(uint32_t v:{0x59544356u,4u,uint32_t(MapSize),uint32_t(MapSize),uint32_t(TileSize),checksum(data)})write(f,v);
...
void CitySimulation::load(World& w,TrafficSimulation& traffic,const std::filesystem::path& path) {
    ...
    if(header[1]<=2){candidateWorld->load(path);candidate.definitions_=definitions_;candidate.refresh(*candidateWorld);}
    else {
        if((header[1]!=3&&header[1]!=4)||header[2]!=MapSize||header[3]!=MapSize||header[4]!=TileSize)throw std::runtime_error("Unsupported city version");
        ...
        std::string payload(size,'\0');file.read(payload.data(),size);if(!file||checksum(payload)!=header[5])throw std::runtime_error("City checksum mismatch");std::istringstream in(payload,std::ios::binary);candidateWorld->readRoads(in);
        ...
        for(auto& d:c.definitions_){d.name=text(in,256);d.cost=read<int>(in);d.upkeep=read<int>(in);d.capacity=read<int>(in);d.unlock=read<int>(in);d.radius=read<int>(in);if(d.cost<0||d.upkeep<0||d.capacity<0||d.capacity>10000000||d.unlock<0||d.radius<0||d.radius>512)throw std::runtime_error("Invalid saved definitions");}
        ...
        for(auto& b:c.buildings_){b.id=read<uint64_t>(in);b.cell.x=read<int>(in);b.cell.z=read<int>(in);b.kind=BuildingKind(read<uint8_t>(in));b.level=read<uint8_t>(in);b.variant=read<uint8_t>(in);b.productivity=read<float>(in);
...
void CitySimulation::validate(const World& w)const {
    ...
    for(const auto& h:households_){auto* b=find(h.home);if(!std::isfinite(h.commuteQuality)||h.commuteQuality<0.25f||h.commuteQuality>1||!std::isfinite(h.travelSeconds)||h.travelSeconds<0||!b||b->kind!=BuildingKind::Residential||!b->level||h.members!=b->residents||h.id>=nextId_||!allIds.insert(h.id).second||!homes.insert(h.home).second)throw std::runtime_error("Invalid household");
        if(h.workplace){auto* job=find(h.workplace);if(!job||(job->kind!=BuildingKind::Commercial&&job->kind!=BuildingKind::Industrial))throw std::runtime_error("Invalid workplace");}}
    for(const auto& b:buildings_)if(b.level&&b.kind==BuildingKind::Residential&&!homes.contains(b.id))throw std::runtime_error("Home missing household");
    for(const auto& t:trips_)if(!t.id||t.id>=nextTrip_||!tripIds.insert(t.id).second||t.kind>6||t.cargo<0||t.cargo>500||t.requested>ticks_)throw std::runtime_error("Invalid owned trip");
}
void CitySimulation::scenario(World& w,TrafficSimulation& traffic,int population){
    ...
    auto add=[&](Cell cell,BuildingKind kind,int level,int residents){Building b;b.id=nextId_++;b.cell=cell;b.kind=kind;b.level=uint8_t(level);b.variant=uint8_t(random()%4);b.residents=residents;b.age=150;b.inventory=kind==BuildingKind::Commercial?100:kind==BuildingKind::Industrial?200:0;
        ids_[b.id]=buildings_.size();parcel_[tile(cell)]=int(buildings_.size());buildings_.push_back(b);if(kind==BuildingKind::Residential&&level)households_.push_back({nextId_++,b.id,0,residents});};
    ...
    for(int g=0;g<groups;++g)for(int k=4;k<int(BuildingKind::Count);++k){Cell c{241+(g%3)*32+(k-4),219+(g/3)*18};if(w.road(c.x,c.z)||at(c))c={251+(g%3)*32+(k-4),219+(g/3)*18};if(!w.road(c.x,c.z)&&!at(c))add(c,BuildingKind(k),1,0);}
    if(population>=10000)stats_.milestone=10000;else if(population>=500)stats_.milestone=500;
    rebuild(w,true);regionalTraffic(w,traffic,true);cityStep(w,traffic);publish(w);
}
}

namespace vc {
void CitySimulation::prepareTrafficStress(World& w){
    for(int z=0;z<MapSize;z+=8)w.stroke({352,z},{511,z},true);
...
void CitySimulation::newCity(World& w,TrafficSimulation& traffic){
    ...
    w.generateScenario(0);w.generateRegionalHighway();TrafficSimulation empty({0,seed,20000,64,true});traffic.swap(empty);
    rebuild(w,true);regionalTraffic(w,traffic,true);publish(w);++revision;
}
